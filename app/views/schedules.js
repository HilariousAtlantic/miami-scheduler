...
function Meet({
  ...
  return (
    <div
      key={crn}
      className={['section', locked && 'locked'].join(' ')}
      style={styles}
      onClick={onClick}>
      <div className="section-info course-name">
        {code} {name}
      </div>
      <div className="section-info course-time">
        {formatTime(start_time)} - {formatTime(end_time)} {hall} {room}
      </div>
      ...
      {slots && <div className="section-slots">{slots.rem} Slots Left</div>}
      <div className="lock">
  ...
  return (
    <div className="class-load">
      ...
      <div className="class-load__form">
        ...
        />
        <button
          onClick={() => handleChange({ min: undefined, max: undefined })}>
          Reset
        </button>
      </div>
    </div>
...
export default class SchedulesView extends Component {
  ...
  toggleLock = crn => {
    this.setState(({ lockedSections, currentScheduleIndex }) => {
      if (lockedSections.includes(crn)) {
        ...
        return {
          lockedSections: [...lockedSections, crn],
          currentScheduleIndex: 0,
        };
      }
    });
  ...
  downloadSchedule() {
    ...
    html2canvas(schedule, { scale: 2.5 }).then(canvas => {
  ...
  render() {
    ...
    const enabledInstructors = Object.keys(instructorFilters).filter(
      instructor => instructorFilters[instructor]
    );
    const schedules = generatedSchedules
      .filter(schedule => {
        return (
          lockedSections.every(crn => schedule.crns.indexOf(crn) !== -1) &&
          (!enabledInstructors.length ||
            enabledInstructors.find(
              instructor => schedule.instructors.indexOf(instructor) !== -1
            )) &&
          Object.keys(classLoads).every(
            day =>
              schedule.meets[day].length >=
                (classLoads[day].min || -Infinity) &&
              schedule.meets[day].length <=
                (classLoads[day].max >= 0 ? classLoads[day].max : Infinity)
          ) &&
          (!filterFullSchedules ||
            schedule.crns.every(crn => !slots[crn] || slots[crn].rem > 0))
        );
      })
    ...
    const prevSchedule = () =>
      this.setState({
        currentScheduleIndex: Math.max(currentScheduleIndex - 1, 0),
      });
      ...
      <div className="view schedules-view">
        <div className="sidebar">
          ...
          <div className="schedule-search">
            <button onClick={prevSchedule}>
              <i className="fa fa-chevron-left" />
            </button>
            <input
              type="text"
              value={`Schedule ${currentScheduleIndex +
                1} of ${schedules.length}`}
              readOnly
            />
            <button onClick={nextSchedule}>
          ...
          <div className="schedule-filters">
            <div className="filter-section">
              ...
              <RadioGroup
                ...
                onChange={e =>
                  this.setState({ schedulesSort: e.target.value })}>
                <Radio
                  value="early"
            ...
            <div className="filter-section">
              ...
              <Check
                ...
                onChange={(_, checked) =>
                  this.setState({ filterFullSchedules: checked })}
                text="Hide Schedule"
              ...
              <Check
                ...
                onChange={(_, checked) =>
                  this.setState({ fadeFullSections: checked })}
                text="Fade Section"
            ...
            <div className="filter-section">
              ...
              <p className="filter-section__info">
                Only schedules that meet within the ranges will show
              </p>
            ...
            <div className="filter-section">
              ...
              <p className="filter-section__info">
                Only schedules with the selected instructors will show
              </p>
              {selectedCourses.map(
                course =>
                  !getUniqueInstructors(course).length ? null : (
                    <div className="instructor-group" key={course.code}>
                      ...
                      <CheckGroup
                        ...
                        defaultValue={false}
                        onChange={instructorFilters =>
                          this.setState({
                            instructorFilters,
                            currentScheduleIndex: 0,
                          })}>
                        {getUniqueInstructors(course)
                          .filter(i => uniqueInstructors[i])
                          .map(instructor => (
                            <Check
                              key={instructor}
                              value={instructor}
                              text={instructor}
                              hint={
                                uniqueInstructors[instructor] + ' Schedules'
                              }
                            />
                          ))}
                      </CheckGroup>
            ...
            <div className="schedule-container">
              <div className="schedule">
                <div className="schedule-header">
                  <span className="schedule-column">
