        ...
        <div className="sidebar">
          ...
          <ul className="course-list">
            {searchedCourses.map(course => (
              <li key={course.id} onClick={() => this.props.onCourseSelect(course)}>
                {course.code} - {course.title}
              </li>
            ))}
          </ul>
          <Link className="button button--primary" to={`/schedules?courses=${courses_query}`} onClick={this.props.onSchedulesGenerate}>
