...
const ScheduleSidebar = styled.div`
  ...
  padding: 16px;
  background: #f5f5f5;
  box-shadow: -2px 0 16px rgba(0, 0, 0, 0.2) inset;
  max-width: 360px;
...
function SelectedCourses({ courses, onCourseClick }) {
  ...
  const credits =
    total.low === total.high ? total.low : `${total.low} - ${total.high}`;
      ...
      <List>
        ...
        {courses.map(course => (
          <ListItem key={course.id}>
            ...
            <i className="fa fa-close" onClick={() => onCourseClick(course)} />
          </ListItem>
        ))}
      </List>
    </SelectedCoursesWrapper>
      ...
      <SplitContainer>
        <CourseLabelList>
          {selectedCourses.map((course, i) => (
            <CourseLabel key={course.id} color={colors[i]}>
              {course.code}
            </CourseLabel>
          ))}
        </CourseLabelList>
...
function ScheduleMeet({
  ...
  return (
    <MeetWrapper start={start} length={length} color={color}>
      ...
      <span>
        {formatTime(start_time)} - {formatTime(end_time)} {room} {hall}
      </span>
    </MeetWrapper>
  );
}
      ...
      <ScheduleCalendar>
        ...
        {days.map(day => (
          <ScheduleDay key={day.sm}>
            {meets[day.sm].map((meet, i) => (
              <ScheduleMeet
                key={day + i}
                {...meet}
                instructor={
                  meet.instructors[0] && meet.instructors[0].last_name
                }
                start={
                  (meet.start_time - start_time) / (end_time - start_time) * 100
                }
                length={
                  (meet.end_time - meet.start_time) /
                  (end_time - start_time) *
                  100
                }
...
export default class Generator extends Component {
  ...
  async componentDidMount() {
    ...
    const { data: { schedules } } = await axios.get(
      'http://beta.miamischeduler.com/api/schedules?courses=201820CSE270M,201820CSE451,201820CSE465,201820ENG313,201820GLG301'
    );
    this.setState({ generatedSchedules: schedules });
    const { data: selectedCourses } = await axios.get(
      'http://beta.miamischeduler.com/api/courses?id=201820CSE270M,201820CSE451,201820CSE465,201820ENG313,201820GLG301'
    );
    ...
    window.addEventListener('scroll', () => {
      ...
      if (
        !showScheduleNavbar &&
        window.scrollY >= this.schedulesContainer.offsetTop
      ) {
  ...
  async fetchTerms() {
    const { data } = await axios.get(
      'http://beta.miamischeduler.com/api/terms'
    );
    const defaultTerm = data.find(term => /(Fall|Spring)/.test(term.name));
    this.setState(
      { terms: data, selectedTerm: defaultTerm.id },
      this.searchCourses
    );
  }
  ...
  async searchCourses() {
    ...
    const { data } = await axios.get(
      `http://beta.miamischeduler.com/api/courses/search?term=${selectedTerm}`
    );
  ...
  render() {
    return (
      <AppWrapper>
          ...
          <CourseSelectorWrapper>
            <SearchForm
              ...
              onTermChange={selectedTerm =>
                this.setState({ selectedTerm }, this.searchCourses)
              }
        ...
        <Container>
          <GenerationStatus>
            <span ref={ref => (this.schedulesContainer = ref)}>
              {this.state.generatedSchedules.length} Schedules Generated
            </span>
          </GenerationStatus>
          <ScheduleGrid
        ...
        {this.state.selectedSchedule && (
          <ModalContainer>
            <ModalClose
              onClose={() => this.setState({ selectedSchedule: null })}
            />
            <ScheduleInformation>
              <DetailedSchedule>
                <ScheduleHeader>
                ...
                <ScheduleCalendar>
                  {days.map(day => (
                    <ScheduleDay key={day.sm}>
                      {this.state.selectedSchedule.meets[day.sm].map(
                        (meet, i) => (
                          <ScheduleMeet
                            key={day + i}
                            {...meet}
                            instructor={
                              meet.instructors[0] &&
                              `${meet.instructors[0].first_name} ${
                                meet.instructors[0].last_name
                              }`
                            }
                            start={(meet.start_time - 480) / 720 * 100}
                            length={
                              (meet.end_time - meet.start_time) / 720 * 100
                            }
                            color={
                              colors[
                                this.state.selectedSchedule.crns.indexOf(
                                  meet.crn
                                )
                              ]
                            }
